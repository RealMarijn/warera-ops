...
- **Market page** — shows buy/sell order counts, item quantity, and total money per item, with troll orders (extreme prices) filtered out.
- **Country/MU account pages** — shows total inventory value (money plus all items at current market price).
- **Toggle button** (top-left corner) to turn all of the above on or off.
